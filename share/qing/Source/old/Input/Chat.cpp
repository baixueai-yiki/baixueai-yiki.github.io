...
// 辅助对话展示函数。
static void QingChatTalk(HWND hwnd, const wchar_t* text)
{
    MessageBoxW(hwnd, text, L"晴小姐", MB_OK);
}

// 特殊对话分支。
static void QingChatDialogLove(HWND hwnd)
{
    QingChatTalk(hwnd, L"……真的？");
    int res = MessageBoxW(hwnd, L"你要让我留下，还是去休息？", L"晴小姐", MB_YESNO);
    if (res == IDYES)
        QingChatTalk(hwnd, L"那我就继续陪着你……");
    else
    {
        QingChatTalk(hwnd, L"……那我先睡一会儿，好吗？");
        PostQuitMessage(0);
    }
}

static void QingChatDialogUnknown(HWND hwnd)
{
    QingChatTalk(hwnd, L"……我不太明白你在说什么。");
}

void QingChatHandleInput(HWND hwnd, const std::wstring& input)
{
    if (input == L"我爱你")
        QingChatDialogLove(hwnd);
    else
        QingChatDialogUnknown(hwnd);
}
...
void QingShowChatInput(HWND hwndParent)
{
    ...
    if (!s_inputText.empty())
        QingChatHandleInput(hwndParent, s_inputText);
}
