...
<details>
  ...
  1. 在 git 创建一个新的库，库的名字为：<git用户名>.github.io，创建时勾选添加 README（添加自述文件）  
  2. 仓库名称 → 背景设定 → 代码与自动化 → 页面（像网页的小图标）  
  3. 构建与部署 → 源 → 从分支部署  
  4. 分支 → 选择分支，默认是 main  
  5. 创建一个 index.html 文件，在里面编辑页面  
  6. 地址为 https://<git用户名>.github.io  

  剩下的界面优化自由发挥，不会的可以问 AI  
